 ...
 *
 * This namespace provides functionality for tokenizing expressions, transforming them
 * to Reverse Polish Notation (RPN) using the Shunting Yard algorithm, and evaluating
 * mathematical expressions. It includes structures for operators, functions, variables,
 * and utilities for handling tokens.
 */
namespace Tokenizer {
 ...
 *
 * This enum defines the character representations for different operators used in
 ...
 *
 * This structure captures the name of the function and a callable that performs
 * the operation on a single double value.
 */
struct UnaryFunction {
 ...
 *
 * This structure includes the name of the function and a callable that performs
 * the operation on two double values.
 */
struct BinaryFunction {
 ...
 *
 * This type can hold a double, a variable, an operator, a unary function, or a binary function.
 */
using TokenType = std::variant<double, Variable, Operator, UnaryFunction, BinaryFunction>;
...
const inline std::unordered_map<Operator, BinaryFunction> op_to_fn = {
    { Operator::Sum, BinaryFunction { "Sum", [](double a, double b) { return a + b; } } },
    { Operator::Sub, BinaryFunction { "Sub", [](double a, double b) { return a - b; } } },
    { Operator::Div, BinaryFunction { "Div", [](double a, double b) {
                                         assert(b != 0);
                                         return a / b;
                                     } } },
    { Operator::Mult, BinaryFunction { "Mult", [](double a, double b) { return a * b; } } },
    { Operator::Pow, BinaryFunction { "Pow", [](double a, double b) { return std::pow(a, b); } } },
};
...
/**
 * @brief Implements the Shunting Yard algorithm to convert infix expressions to RPN.
 *
