...
std::ostream& Tokenizer::operator<<(std::ostream& out, const Tokenizer::Operator& op)
{
    out << static_cast<char>(op);
    return out;
}

std::ostream& Tokenizer::operator<<(std::ostream& out, const Tokenizer::UnaryFunction& func)
{
    out << func.name;
    return out;
}

std::ostream& Tokenizer::operator<<(std::ostream& out, const Tokenizer::BinaryFunction& func)
{
    out << func.name;
    return out;
}

std::ostream& Tokenizer::operator<<(std::ostream& out, const Tokenizer::Variable& var)
{
...
std::ostream& Tokenizer::operator<<(std::ostream& out, const std::queue<TokenType>& q)
{
    auto cp = q;
...
Tokenizer::Associativity Tokenizer::getAssociativity(const Tokenizer::Operator& op)
{
    using namespace Tokenizer;
    if (op == Operator::LParen || op == Operator::RParen) {
...
auto Tokenizer::tokenize(const std::string_view expression) -> std::vector<TokenType>
{
    std::cout << expression << '\n';
...
inline std::vector<Tokenizer::TokenType> Tokenizer::shunting_yard(const std::string& expression)
{
    ...
    for (const auto& token : tokens) {
        if (isNumber(token)) {
            ...
            if (isOperatorButNotAParen(op)) {
                ...
                while ((not op_stack.empty() && op_stack.top() != Operator::LParen) and ((getOperatorPrecedence(op_stack.top()) > getOperatorPrecedence(o1)) or ((getOperatorPrecedence(op_stack.top()) == getOperatorPrecedence(o1)) and getAssociativity(o1) == Associativity::Left))) {
